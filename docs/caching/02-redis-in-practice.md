...
- Choosing Redis data structures deliberately (hash vs. string, sorted
  ...
  and takes advantage of Redis's atomic, purpose-built operations. For
  example, a leaderboard built on a sorted set (`ZADD`/`ZRANGE`) gets
  ranking for free, versus reimplementing sort logic over a plain list.
- Redis's default in-memory nature means a Redis outage or restart can
  ...
  assuming Redis as an always-available, always-durable store. For
  example, a session store backed only by Redis with no persistence
  configured can log every user out on a restart if the app treats Redis
  as durable truth instead of a cache.
- Distributed locks built on Redis (`SET NX EX`) are simple but have known
  ...
  locking. For example, a lock expiring while the holder is still slowly
  processing (GC pause, slow I/O) can let a second client acquire the same
  lock and both run concurrently.
