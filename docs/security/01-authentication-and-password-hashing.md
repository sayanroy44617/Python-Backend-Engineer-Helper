...
- Authentication design decisions (session vs token, MFA requirements,
  ...
  one. For example, "disable all refresh tokens issued before 14:05 UTC"
  is an operationally actionable response; "wait for old JWTs to expire"
  may be too slow during an active incident.
- Migrating a password-hashing algorithm across a live user base (e.g.
  ...
  avoids a painful later transition. For example, a user with a bcrypt
  cost 10 hash logs in successfully, and the app immediately rewrites it
  as argon2 before returning the session.
- Credential storage and authentication are consistently among the
  ...
  senior engineer. For example, using `bcrypt.checkpw()` from a reviewed
  library is the right trade-off; writing a custom "salt + SHA-256 +
  pepper" helper is not.
